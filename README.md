...
- Machine 2 入口图标支持 `Roll / Down Roll` 动画控制
- 机器页：`3x3` 老虎机、5 条 Payline、Spin、加减下注、奖励结算、规则弹窗、返回大厅
- Machine 3：整轴向下状态机滚动（加速->匀速->减速->回调->停止），中奖线按小额到大额循环播放
- 机台编辑模式：支持显示 `Hierarchy` 与 `Inspector` 面板，并支持窗口关闭/拖拽
